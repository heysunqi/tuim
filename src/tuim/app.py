...
from tuim.config import load_connections, save_connections, get_config_path
from tuim.i18n import t, t_en
...
from tuim.widgets.k8s_resource_view import K8sResourceView
from tuim.widgets.status_bar import StatusBar
from tuim.widgets.command_bar import CommandBar
...
class TuimApp(App):
    """Tuim - TUI Remote Connection Manager."""
    ...
    async def _enter_k8s_browser(self, conn):
        ...
        k8s_view.set_k8s_context(
            ...
            token=k8s_cfg.token if k8s_cfg else "",
            insecure_skip_tls_verify=k8s_cfg.insecure_skip_tls_verify if k8s_cfg else False,
            host=conn.host,
            port=conn.port,
        )
        ...
        try:
            ...
            if "timeout" in error_msg.lower():
                self.notify(t("k8s_connect_timeout"), severity="error", timeout=8)
            else:
                self.notify(t("k8s_connect_failed", error=error_msg), severity="error", timeout=8)
            self._k8s_connection = None
            self._return_to_list()

    async def _k8s_safe_load(self, k8s_view, resource_type):
        # type: (object, str) -> None
        """Load K8s resources with error handling (in-browser context)."""
        try:
            await k8s_view.load_resources(resource_type)
        except Exception as exc:
            error_msg = str(exc)
            if "timeout" in error_msg.lower():
                self.notify(t("k8s_connect_timeout"), severity="error", timeout=8)
    ...
    async def _execute_k8s_command(self, cmd):
        ...
        from tuim.services.k8s_service import RESOURCE_ALIASES
        ...
        if cmd == "ns":
            await self._k8s_safe_load(k8s_view, "namespaces")
            return
        if cmd.startswith("ns "):
            ns_name = cmd[3:].strip()
            if ns_name:
                k8s_view.set_namespace(ns_name)
                await self._k8s_safe_load(k8s_view, k8s_view.get_current_resource_type())
            return

        # Resource type alias
        if cmd in RESOURCE_ALIASES:
            await self._k8s_safe_load(k8s_view, cmd)
            return
    ...
    async def _k8s_exec_pod(self):
        ...
        import copy
        from tuim.models import K8sConfig
        ...
        if resource_type == "namespaces":
            # Enter on a namespace → switch to that namespace, restore previous resource type
    ...
    def _prompt_shell_retry(self):
        ...
        from tuim.screens.shell_picker import ShellPickerScreen
    ...
    async def _k8s_run_command(self, kubectl_args, status_label, message, on_disconnect=None):
        ...
        base_cmd = ["kubectl"]
        base_cmd.extend(cfg.kubectl_auth_args(exec_conn.host, exec_conn.port))
        if cfg.namespace:
