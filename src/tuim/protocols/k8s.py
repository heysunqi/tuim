...
class K8sHandler(ProtocolHandler):
    ...
    def _build_exec_command(self) -> List[str]:
        ...
        cmd = ["kubectl"]
        cmd.extend(cfg.kubectl_auth_args(self.connection.host, self.connection.port))
        if cfg.namespace:
    ...
    async def _preflight_check(self, cfg):
        # type: (...) -> bool
        """Quick cluster connectivity check before forking PTY.

        Runs `kubectl cluster-info` with a short timeout. Returns True if
        the cluster is reachable, False otherwise (emits error + disconnect).
        """
        ...
        try:
            ...
            if proc.returncode != 0:
                ...
                msg = t("k8s_connect_failed", error=err)
                logger.error(msg)
            ...
            msg = t("k8s_connect_timeout")
            logger.error(msg)
            self._emit_output(msg + "\r\n")
            self._emit_disconnect()
            return False
        except Exception as exc:
            msg = t("k8s_connect_failed", error=str(exc))
            logger.error(msg)
            self._emit_output(msg + "\r\n")
            self._emit_disconnect()
            return False
    ...
    async def connect(self) -> None:
        ...
        # Pre-flight check: verify cluster connectivity before forking a PTY
        # (which would hang indefinitely on unreachable clusters)
        cfg = self.connection.k8s_config
        if cfg is not None:
            preflight_ok = await self._preflight_check(cfg)
            if not preflight_ok:
                return
