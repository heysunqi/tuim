...
class K8sConnectionError(Exception):
    """Raised when kubectl cannot connect to the cluster."""
    pass
...
class K8sService:
    ...
    def _base_args(self):
        ...
        if self.token:
            ...
            if self.port:
                args.extend(["--server", "{}://{}:{}".format(scheme, effective_host, self.port)])
            else:
    ...
    async def get_resources(self, resource_type):
        ...
        try:
            ...
            raise K8sConnectionError("timeout")
        except K8sConnectionError:
            raise
        except Exception as exc:
            logger.error("kubectl error: %s", exc)
            raise K8sConnectionError(str(exc))
