...
class AddConnectionScreen(ModalScreen[Optional[Connection]]):
        ...
        with Vertical():
            ...
            with VerticalScroll():
                ...
                with Vertical(id="k8s-fields", classes="protocol-fields hidden"):
                    with Vertical(classes="form-group"):
                        yield Label(t("label_kubeconfig"))
                        yield Input(
                            value=self._get_k8s_val("kubeconfig"),
                            placeholder="~/.kube/config",
                            id="field-k8s-kubeconfig",
                        )
                    with Vertical(classes="form-group"):
                        yield Label(t("label_token"))
                        yield Input(
                            value=self._get_k8s_val("token"),
                            placeholder=t("placeholder_token"),
                            password=True,
                            id="field-k8s-token",
                        )
                    with Vertical(classes="form-group"):
                        yield Label(t("label_skip_tls"))
                        yield Select(
                            options=[("否 / No", "no"), ("是 / Yes", "yes")],
                            value="yes" if self._get_k8s_bool("insecure_skip_tls_verify") else "no",
                            id="field-k8s-skip-tls",
                        )
                    with Vertical(classes="form-group"):
    ...
    def _get_k8s_bool(self, attr):
        # type: (str) -> bool
        conn = self.editing_connection
        if conn and conn.k8s_config:
            return bool(getattr(conn.k8s_config, attr, False))
        return False
    ...
    def _show_protocol_fields(self, protocol_value):
        ...
        # Update host label based on protocol
        try:
            host_container = self.query_one("#field-host", Input).parent
            if host_container is not None:
                label = host_container.query_one(Label)
                if protocol_value == Protocol.K8S.value:
                    label.update(t("label_host_k8s"))
                else:
                    label.update(t("label_host"))
        except Exception:
            pass
    ...
    def _save(self):
        ...
        # Host is required for non-K8s protocols; for K8s token mode host is also required
        ...
        if protocol == Protocol.K8S:
            k8s_token = self.query_one("#field-k8s-token", Input).value.strip()
            if k8s_token and not host:
                self.notify(t("err_k8s_token_needs_host"), severity="error")
                self.query_one("#field-host", Input).focus()
                return
        ...
        if protocol == Protocol.SSH:
            ...
            skip_tls_val = self.query_one("#field-k8s-skip-tls", Select).value
            k8s_config = K8sConfig(
                kubeconfig=self.query_one("#field-k8s-kubeconfig", Input).value.strip(),
                token=self.query_one("#field-k8s-token", Input).value.strip(),
                insecure_skip_tls_verify=(skip_tls_val == "yes"),
