...
Tuim is a k9s-style TUI remote connection manager for SSH, RDP, VNC, Telnet and Kubernetes, built on Python's Textual framework. It uses vim-style keyboard navigation and a dark theme. UI strings are in Chinese.
...
# Run
tuim                          # default config: ~/.config/tuim/connections.yaml
tuim --config path/to.yaml    # custom config
python -m tuim                # alternative
...
`TuimApp` (`app.py`) is the central orchestrator. All key events flow through `on_key()` which dispatches to the active mode handler. Modal screens (add/edit/delete/help/shell picker) are pushed onto Textual's screen stack.
...
**Config**: YAML at `~/.config/tuim/connections.yaml`. First-run copies from `config/connections.example.yaml`.
...
- Build backend is hatchling with src-layout (`src/tuim/`)
